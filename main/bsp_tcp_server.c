...
#endif
            // bsp_uart_send((uint8_t*)rx_buffer, len);
        }
        memset(rx_buffer, 0, TCP_TX_BUF_LEN);
    } while (len > 0);
}
...
void bsp_tcp_server_task_init(void)
{
    ...
    ESP_LOGI(TAG, "TCP_Server is running...\n");
...
int8_t bsp_tcp_send(char* txBuf, int len)
{
    ...
    while (to_write > 0) {
        int written = send(g_socket, txBuf + (len - to_write), to_write, 0);
        if (written < 0) {
            ESP_LOGE(TAG, "Error occurred during sending: errno %d", errno);
        }
        to_write -= written;
